...
with tempfile.TemporaryDirectory() as td:
    subprocess.run([sys.executable,str(root/'run_stagewise_coverage.py'),str(root/'reported_public_event_input.csv'),'--output-dir',td],check=True,capture_output=True,text=True)
...
for p in x['policies']: assert p['captured']==expected[p['K']]
...
assert abs(p['source_stage_share_of_misses']-274/708)<1e-12 and abs(p['post_source_share_of_misses']-434/708)<1e-12
