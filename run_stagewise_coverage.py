...
def load_rows(path):
    with _open_text(path) as fh:
        ...
        for n,row in enumerate(reader,start=2):
            ...
            seen.add(oid)
            obs=parse_bool(row["source_observable"],"source_observable",n)
            rank=parse_rank(row["best_conservative_rank_within_horizon"],n)
            if not obs and rank is not None:
                raise ValueError(f"row {n}: source-unobservable outcome carries a finite rank")
            top1=None
            if "top1pct_captured" in row and (row.get("top1pct_captured") or "").strip():
                top1=parse_bool(row["top1pct_captured"],"top1pct_captured",n)
...
def compute(rows,capacities):
    ...
    for K in capacities:
        ...
        result["policies"].append({
            "policy":f"Top-{K}","K":K,"captured":captured,"total_misses":misses,
            "source_unobservable_misses":NU,"observable_not_captured":visible_miss,
            "conditional_capture":captured/NO if NO else None,
            "full_denominator_coverage":captured/N,
            "source_stage_share_of_misses":NU/misses if misses else None,
            "post_source_share_of_misses":visible_miss/misses if misses else None
        })
    vals=[r["top1"] for r in rows if r["top1"] is not None]
    if vals and len(vals)==N:
        c=sum(vals)
        result["top1pct"]={"captured":c,"conditional_capture":c/NO if NO else None,"full_denominator_coverage":c/N}
    return result
...
def write_outputs(result,outdir):
    ...
    with (outdir/"miss_location_by_capacity.csv").open("w",newline="",encoding="utf-8") as fh:
        ...
        w=csv.DictWriter(fh,fieldnames=fields); w.writeheader(); w.writerows(result["policies"])

def main():
    ap=argparse.ArgumentParser()
    ap.add_argument("input_csv",type=Path)
    ap.add_argument("--output-dir",type=Path,default=Path("coverage_output"))
    ap.add_argument("--capacities",type=int,nargs="+",default=[100,500,1000])
    args=ap.parse_args()
